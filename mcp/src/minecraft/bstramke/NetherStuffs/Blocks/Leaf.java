...
public class Leaf extends Block implements IShearable {
	...
	public Leaf(int par1) {
		...
		setUnlocalizedName("NetherLeaves");
		for (int i = 0; i < LeafItemBlock.getMetadataSize(); i++) {
			LanguageRegistry.instance().addStringLocalization("tile.NetherLeaves." + LeafItemBlock.blockNames[i] + ".name", LeafItemBlock.blockDisplayNames[i]);
		}
		setHardness(0.2F);
		setLightOpacity(1);
	}
